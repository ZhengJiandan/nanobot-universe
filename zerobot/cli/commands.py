...
from zerobot import __version__, __logo__
from zerobot.config.schema import Config

app = typer.Typer(
    name="zerobot",
    help=f"{__logo__} zerobot - Personal AI Assistant",
...
# Sub-apps
from zerobot.cli.universe import app as universe_app  # noqa: E402
...
def _init_prompt_session() -> None:
    ...
    history_file = Path.home() / ".zerobot" / "history" / "cli_history"
...
def version_callback(value: bool):
    if value:
        console.print(f"{__logo__} zerobot v{__version__}")
        raise typer.Exit()
...
def onboard():
    ...
    from zerobot.config.loader import get_config_path, load_config, save_config
    from zerobot.config.schema import Config
    from zerobot.utils.helpers import get_workspace_path
    ...
    console.print(f"\n{__logo__} zerobot is ready!")
    console.print("\nNext steps:")
    console.print("  1. Add your API key to [cyan]~/.zerobot/config.json[/cyan]")
    console.print("     Get one at: https://openrouter.ai/keys")
    console.print("  2. Chat: [cyan]zerobot agent -m \"Hello!\"[/cyan]")
    console.print("\n[dim]Want Telegram/WhatsApp? See: https://github.com/HKUDS/zerobot#-chat-apps[/dim]")
...
I am zerobot, a lightweight AI assistant.
...
def gateway(
    ...
    """Start the zerobot gateway."""
    from zerobot.config.loader import load_config, get_data_dir
    ...
    from zerobot.cron.types import CronJob
    from zerobot.heartbeat.service import HeartbeatService
    
    if verbose:
        import logging
        logging.basicConfig(level=logging.DEBUG)
    
    console.print(f"{__logo__} Starting zerobot gateway on port {port}...")
...
def agent(
    ...
    def _thinking_ctx():
        ...
        return console.status("[dim]zerobot is thinking...[/dim]", spinner="dots")
...
def _get_bridge_dir() -> Path:
    ...
    user_bridge = Path.home() / ".zerobot" / "bridge"
    ...
    pkg_bridge = Path(__file__).parent.parent / "bridge"  # zerobot/bridge (installed)
    ...
    if not source:
        ...
        console.print("Try reinstalling: pip install --force-reinstall zerobot")
        raise typer.Exit(1)
...
def cron_add(
    ...
    from zerobot.config.loader import get_data_dir
    from zerobot.cron.service import CronService
    from zerobot.cron.types import CronSchedule
...
def status():
    ...
    console.print(f"{__logo__} zerobot Status\n")
