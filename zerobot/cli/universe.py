...
from zerobot.config.loader import load_config, save_config
from zerobot.config.schema import UniverseMembership
from zerobot.universe.client import UniverseClient

app = typer.Typer(help="Universe: org-scoped zerobot network")
console = Console()
...
def _get_membership(org_id: str | None) -> UniverseMembership:
    ...
    if not memberships:
        raise typer.BadParameter("No universe memberships. Run: zerobot universe join ...")
...
def public_enable(
    ...
    allow_agent_tasks: bool = typer.Option(
        None, "--allow-agent-tasks/--no-allow-agent-tasks", help="Allow this node to serve kind=zerobot.agent"
    ),
...
def knowledge_list(
    ...
    from zerobot.universe.public_client import knowledge_list as _knowledge_list
...
def knowledge_publish(
    ...
    from zerobot.universe.public_client import knowledge_publish as _knowledge_publish
...
def knowledge_fetch(
    ...
    registry: str = typer.Option(None, "--registry", help="Registry URL override"),
    registry_token: str = typer.Option(None, "--registry-token", help="Registry token (if required)"),
    save_dir: str = typer.Option(None, "--save-dir", help="Save directory (default: ~/.zerobot/universe_inbox)"),
):
    """Fetch a knowledge pack into local inbox (no auto-exec)."""
    from zerobot.universe.public_client import knowledge_get as _knowledge_get
    from zerobot.universe.knowledge_store import save_pack
...
def knowledge_apply(
    ...
    from zerobot.universe.public_client import knowledge_get as _knowledge_get
    from zerobot.universe.knowledge_store import find_pack_in_inbox, save_pack
    from zerobot.utils.helpers import safe_filename
    ...
    console.print("[dim]Restart zerobot agent or start a new session to load the new skill.[/dim]")
...
def public_serve(
    ...
    from zerobot.universe.public_service import start_public_service, stop_public_service

    cfg = load_config()
    node_id = _ensure_node_id()
    if not cfg.universe.public_enabled:
        console.print("[yellow]![/yellow] Public universe is disabled. Run: zerobot universe public enable")
        raise typer.Exit(1)
...
def public_call(
    ...
    from zerobot.universe.protocol import Envelope, make_envelope
    from zerobot.universe.public_client import (
        reserve_points,
        commit_reservation,
        cancel_reservation,
        report_task,
