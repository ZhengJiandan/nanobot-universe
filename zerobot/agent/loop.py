...
from zerobot.agent.tools.filesystem import ReadFileTool, WriteFileTool, EditFileTool, ListDirTool
from zerobot.agent.tools.shell import ExecTool
from zerobot.agent.tools.web import WebSearchTool, WebFetchTool
...
class AgentLoop:
    ...
    def __init__(
        ...
        from zerobot.config.schema import ExecToolConfig
        from zerobot.config.schema import UniverseConfig
    ...
    async def _connect_mcp(self) -> None:
        ...
        from zerobot.agent.tools.mcp import connect_mcp_servers
        ...
        for cap in required_caps:
            try:
                node, out = await delegate_task(
                    ...
                    kind=getattr(uc, "public_auto_delegate_kind", "zerobot.agent"),
                    prompt=prompt,
    ...
    async def _infer_required_caps_async(
        ...
        default_kind = getattr(uc, "public_auto_delegate_kind", "zerobot.agent")
    ...
    async def _auto_pull_knowledge(self, provider_node: str, cap: str | None) -> None:
        ...
        try:
            from zerobot.universe.public_client import knowledge_list, knowledge_get
